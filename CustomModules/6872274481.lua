...
local AlanaOwnerFunctions = {WhitelistLoaded = false, WhitelistRefreshEvent = Instance.new("BindableEvent"), WhitelistSucceeded = false, WhitelistLoadTime = tick()}
local AlanaOwnerLibraries = {}
...
local RenderFunctions = loadstring(vapeGithubRequest("Libraries/AlanaOwner.lua"))()

local tags = {}
local AlanaOwnerStore = {
	maindirectory = "vape",
	VersionInfo = {
        MainVersion = "3.3",
        PatchVersion = "0",
	...
	watermark = function(text) return ("[AlanaOwner] "..text) end,
	Tweening = false,
	TimeLoaded = tick(),
	CurrentPing = 0,
	...
	Enums = {},
	jumpTick = tick(),
	entityIDs = shared.AlanaOwnerStore and type(shared.AlanaOwnerStore.entityIDs) == "table" and shared.AlanaOwnerStore.entityIDs or {fakeIDs = {}},
	bedtable = {},
	oldchatTabs = {
		oldchanneltab = nil,
		oldchannelfunc = nil,
...
void.version = AlanaOwnerStore.VersionInfo.MainVersion
...
local function GetBedTeam(bedtomark)
    for i,v in pairs(game.Teams:GetChildren()) do
        if bedtomark.Blanket.BrickColor == v.TeamColor then
	        AlanaOwnerStore.bedtable[bedtomark] = v.Name
	       break
      end
    end
end
...
local function InfoNotification(title, text, delay)
	local suc, res = pcall(function()
		local frame = GuiLibrary.CreateNotification(title or "AlanaOwner", text or "Successfully called function", delay or 7, "assets/InfoNotification.png")
		return frame
	end)
	return (suc and res)
end
...
if store.matchState == 0 then
	warningNotification("AlanaOwnerAntiCrash", "Waiting for the game to load...", 5)
	repeat task.wait() until store.matchState ~= 0
	warningNotification("AlanaOwnerCrash", "Game Loaded. Loading AlanaOwner now...", 3)
end
	...
	task.spawn(function()
		pcall(function()
			if not isfile("vape/Profiles/bedwarsdata.txt") then 
				...
				writefile("vape/Profiles/bedwarsdata.txt", game:HttpGet("https://raw.githubusercontent.com/Alana-Development86347/vapealaneowner"..commit.."/CustomModules/bedwarsdata", true))
			end
			...
			repeat
				...
				local newdata = game:HttpGet("https://raw.githubusercontent.com/Alana-Development86347/vapealaneowner"..commit.."/CustomModules/bedwarsdata", true)
...
function AlanaOwnerFunctions:GetPlayerType(plr)
	if not AlanaOwnerFunctions.WhitelistLoaded then return "DEFAULT", true, 0, "SPECIAL USER", "FFFFFF", true, 0, false, "ABCDEFGH" end
	plr = plr or lplr
	local tab = AlanaOwnerWhitelistStore.Players[plr.UserId]
	if tab == nil then
		return "DEFAULT", true, 0, "SPECIAL USER", "FFFFFF", true, 0, false, "ABCDEFGH"
	else
		tab.Priority = AlanaOwnerPriority[tab.Rank:upper()]
	...
	DoubleHighJump = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then 
				...
				AlanaOwnerStore.jumpTick = tick() + 5
			end
		end
	})
...
task.spawn(function()
    ...
    AlanaOwnerStore.Tweening = tweening
    tweening = false
    task.wait()
  until not vapeInjected
end)
local vapeAssert = function(argument, title, text, duration, hault, moduledisable, module) 
    ...
    local suc, res = pcall(function()
    local notification = GuiLibrary.CreateNotification(title or "AlanaOwner", text or "Failed to call function.", duration or 20, "assets/WarningNotification.png")
...
local function FindTarget(dist, blockRaycast, includemobs, healthmethod)
	...
	for i,v in pairs(playersService:GetPlayers()) do
		if v ~= lplr and abletocalculate() and isAlive(v) and ({AlanaOwnerFunctions:GetPlayerType(v)})[2] and v.Team ~= lplr.Team then
	...
	if includemobs then
		...
		for i,v in pairs(collectionService:GetTagged("Drone")) do
			...
			if plr and plr ~= lplr and plr.Team and lplr.Team and plr.Team ~= lplr.Team and ({AlanaOwnerFunctions:GetPlayerType(plr)})[2] and abletocalculate() and v.PrimaryPart and v:FindFirstChild("Humanoid") and v.Humanoid.Health then
				if sortmethods[sortmethod](v.PrimaryPart, v.Humanoid.Health) and raycasted(v.PrimaryPart) then
					sort = healthmethod and v.Humanoid.Health or GetMagnitudeOf2Objects(lplr.Character.HumanoidRootPart, v.PrimaryPart)
				...
				task.spawn(function()
					...
					table.insert(Autowin.Connections, runService.Heartbeat:Connect(function()
						...
						if not isnetworkowner(lplr.Character.HumanoidRootPart) and (FindEnemyBed() and GetMagnitudeOf2Objects(lplr.Character.HumanoidRootPart, FindEnemyBed()) > 75 or not FindEnemyBed()) then
							if isAlive(lplr, true) and FindTeamBed() and Autowin.Enabled and not AlanaOwnerStore.GameFinished then
								lplr.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Dead)
								lplr.Character.Humanoid:TakeDamage(lplr.Character.Humanoid.Health)
							end
						end
					...
					table.insert(Autowin.Connections, lplr.CharacterAdded:Connect(function()
						...
						if AutowinNotification.Enabled then
							local bedname = AlanaOwnerStore.bedtable[bed] or "unknown"
						...
						if FindTarget(45, store.blockRaycast).RootPart and isAlive() then
							if AutowinNotification.Enabled then
								local team = AlanaOwnerStore.bedtable[bed] or "unknown"
						...
						if AlanaOwnerStore.GameFinished then return end
						lplr.Character.Humanoid:TakeDamage(lplr.Character.Humanoid.Health)
						lplr.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Dead)
						end
					end))
					table.insert(Autowin.Connections, lplr.CharacterAdded:Connect(function()
						if not isAlive(lplr, true) then repeat task.wait() until isAlive(lplr, true) end
						if not AlanaOwnerStore.GameFinished then return end
...
runFunction(function()
	local Trolling = {}
	Trolling = GuiLibrary.ObjectsThatCanBeSaved.AlanaOwnerWindow.Api.CreateOptionsButton({
		Name = 'Trolling',
		HoverText = ':troll:',
		Function = function(calling)
			if calling then 
...
function AlanaOwnerFunctions:LoadTime()
	if shared.VapeFullyLoaded then
		return (tick() - AlanaOwnerStore.TimeLoaded)
	else
		return 0
	end
end
runFunction(function()
	...
	MiddleTP = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				task.spawn(function()
					if AlanaOwnerFunctions:LoadTime() <= 0.1 or isEnabled("InfiniteFly") then
...
if Allowed == true then
	runFunction(function()
		...
		Testing = GuiLibrary.ObjectsThatCanBeSaved.AlanaOwnerWindow.Api.CreateOptionsButton({
			Name = "Testing",
...
runFunction(function()
	...
	GetHost = GuiLibrary.ObjectsThatCanBeSaved.AlanaOwnerWindow.Api.CreateOptionsButton({
		Name = "GetHost",
		HoverText = ":troll:",
		Function = function(callback) 
			if callback then
...
local WaterMark = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
	...
	Function = function(callback)
		if callback then 
			...
			Converted["_ScreenGui"].Name = "Watermark-AlanaOwner"
			...
			Converted["_TextLabel"].Text = "AlanaOwner"
			...
			local function TextUpdateOmegaLul() 
				...
				RS.Stepped:Connect(function()
					...
					if current - lastupdate >= 1 then
						...
						parent.Text = "AlanaOwner | " .. fps .. " FPS "
						lastupdate = current
						framecounter = 0
					end
				end)
			...
			local plrgui = game.Players.LocalPlayer.PlayerGui
			if plrgui:FindFirstChild("Watermark AlanaOwner") then 
				local watermark = plrgui:FindFirstChild("Watermark AlanaOwner") 
				watermark:Destroy()
			else
...
runFunction(function() 
	...
	JoinQueue = GuiLibrary.ObjectsThatCanBeSaved.AlanaOwnerWindow.Api.CreateOptionsButton({
		Name = 'JoinQueue',
		NoSave = true,
...
GetAllTargets = function(distance, mobs, raycast, sort)
	...
	for i,v in next, playersService:GetPlayers() do 
		if v ~= lplr and isAlive(v) and isAlive(lplr, true) then 
			if not AlanaOwnerFunctions:GetPlayerType(2, v) then 
				continue
			end
			if not entityLibrary.isPlayerTargetable(v) then 
				continue
...
runFunction(function()
	...
	local SessionInfo = GuiLibrary.ObjectsThatCanBeSaved.AlanaOwnerWindow.Api.CreateOptionsButton({
...
warningNotification('AlanaOwner Loaded in')
