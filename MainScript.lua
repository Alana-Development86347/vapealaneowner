...
local function vapeGithubRequest(scripturl)
	if not isfile("vape/"..scripturl) then
		...
		suc, res = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/Alana-Development86347/vapealaneowner"..readfile("vape/commithash.txt").."/"..scripturl, true) end)
		...
		displayErrorPopup("Vape has detected uncached files, If you have CustomModules click no, else click yes.", {No = function() end, Yes = function()
			...
			for i,v in pairs(listfiles("vape/CustomModules")) do 
				if isfile(v) and not readfile(v):find("--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.") then
					local last = v:split('\\')
					last = last[#last]
					local suc, publicrepo = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/Alana-Development86347/vapealaneowner"..readfile("vape/commithash.txt").."/CustomModules/"..last) end)
...
local AlanaOwner = GuiLibrary.CreateWindow({
	Name = "AlanaOwner", 
...
GUI.CreateButton({
	Name = "AlanaOwner", 
	Function = function(callback) AlanaOwner.SetVisible(callback) end, 
	Icon = "vape/assets/UtilityIcon.png", 
	IconSize = 17
})
GUI.CreateButton({
...
OnlineProfilesButton.MouseButton1Click:Connect(function()
	...
	if not profilesLoaded then
		...
		for i2,v2 in pairs(onlineprofiles) do
			local profileurl = ""..saveplaceid.."/"..v2.OnlineProfileName
...
local windowSortOrder2 = {"Combat", "Blatant", "Render", "Utility", "World", "AlanaOwner"}
...
local teleportConnection = playersService.LocalPlayer.OnTeleport:Connect(function(State)
    if (not teleportedServers) and (not shared.VapeIndependent) then
		...
		local teleportScript = [[
			loadstring(game:HttpGet("https://raw.githubusercontent.com/Alana-Development86347/vapealaneowner"..readfile("vape/commithash.txt").."/NewMainScript.lua", true))() 
		]]
	...
	if not shared.VapeIndependent then
		...
		if isfile("vape/CustomModules/"..game.PlaceId..".lua") then
			...
			if not shared.VapeDeveloper then
				local suc, publicrepo = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/Alana-Development86347/vapealaneowner"..readfile("vape/commithash.txt").."/CustomModules/"..game.PlaceId..".lua") end)
				if suc and publicrepo and publicrepo ~= "404: Not Found" then
